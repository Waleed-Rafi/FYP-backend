...
def video_concat(processed_file_savepath, name, video_names, audio_path):
    print("Entering video_concat function ...")
    print("processed_file_savepath: ", processed_file_savepath)
    print("name: ", name)
    ...
    for video_name in video_names:
        cmd += [
            "-i",
            "'" + str(os.path.join(processed_file_savepath, video_name + ".mp4")) + "'",
        ]

    cmd += [
        "-filter_complex hstack=inputs=" + str(num_inputs),
        "'" + str(os.path.join(processed_file_savepath, name + ".mp4")) + "'",
        "-loglevel error -y",
    ]
    cmd = " ".join(cmd)
    os.system(cmd)
    ...
    video_add_audio("G_Pose_Driven_", audio_path, processed_file_savepath)


def video_add_audio(name, audio_path, processed_file_savepath):
    print("Entering video_add_audio function ...")
    print("name: ", name)
    print("audio_path: ", audio_path)
    print("processed_file_savepath: ", processed_file_savepath)
    print()
    ...
    cmd = [
        "ffmpeg",
        "-i",
        "'" + os.path.join(processed_file_savepath, name + ".mp4") + "'",
        "-i",
        audio_path,
        "-q:v 0",
        "-strict -2",
...
def main():
    ...
    for clip_idx, path_label in enumerate(lines):
        try:
            ...
            print()
            # path_label = "./misc/Input/0003 1 ./misc/Pose_Source/1234 348 ./misc/Audio_Source/001.mp3 ./misc/Mouth_Source/001 176 None"
            # print()
            # print("path_label_new: ", path_label)
            # print("model: ", model)
